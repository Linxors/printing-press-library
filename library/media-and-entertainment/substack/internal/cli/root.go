...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "substack-pp-cli",
		Short: `Run your Substack growth loop from the terminal — publish, schedule, engage, and measure with cross-table joins no single Substack API call can.`,
		Long: `Run your Substack growth loop from the terminal — publish, schedule, engage, and measure with cross-table joins no single Substack API call can.
...
Health check: run 'substack-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("substack-pp-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newSubsCmd(flags))
	...
	rootCmd.AddCommand(newSectionsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	// Phase 3 transcendence groups (hand-authored novel commands).
	rootCmd.AddCommand(newGrowthCmd(flags))
	rootCmd.AddCommand(newEngageCmd(flags))
	rootCmd.AddCommand(newVoiceCmd(flags))
	rootCmd.AddCommand(newRecsCmd(flags))
	// Top-level cross-publication portfolio commands ported from substack-creator.
